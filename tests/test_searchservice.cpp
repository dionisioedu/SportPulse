...
class FakeApiClient : public IApiClient {
    ...
    const std::string searchPlayersData = R"({
      "player": [
        {
          "idPlayer": "34145395",
          "idTeam": "133619",
    ...
    const std::string searchEventData = R"({
        "event": [
          {
            "idEvent": "2070119",
          ...
          },
          {
            ...
            "strEvent": "Arsenal vs Chelsea",
            "strEventAlternate": "Chelsea @ Arsenal",
            "strFilename": "English Premier League 2024-04-23 Arsenal vs Chelsea",
            "strSport": "Soccer",
            "idLeague": "4328",
            "strLeague": "English Premier League",
            ...
            "dateEvent": "2024-04-23",
            "dateEventLocal": "2024-04-23",
            "strTime": "19:00:00",
            ...
            "strTweet2": "",
            "strTweet3": "",
            "strVideo": "https://www.youtube.com/watch?v=ZzjrcW3QD8Y",
            "strStatus": "Match Finished",
            "strPostponed": "no",
            "strLocked": "unlocked"
          }
    ...
    const std::string searchVenuesData = R"({
        "venues": [
          {
            "idVenue": "19835",
            "idDupe": null,
            ...
            "strDescriptionEN": "Wembley Arena (originally the Empire Pool and, since 1 July 2014, currently known as The SSE Arena, Wembley for sponsorship reasons) is an indoor arena adjacent to Wembley Stadium in Wembley, London. Used for music, comedy, family entertainment and sport, the 12,500-seat facility is London's second-largest indoor arena after The O2 Arena, and the ninth-largest (as of August 2019) in the United Kingdom.",
            "strArchitect": "",
            "intCapacity": "12500",
            "strCost": "",
          ...
          },
          {
            ...
            "strVenueAlternate": null,
            "strVenueSponsor": null,
            "strSport": "Soccer",
            ...
            "strCost": null,
            "strCountry": "Jamaica",
            "strLocation": "Hayes, Clarendon, Jamaica",
            "strTimezone": null,
            "intFormedYear": "0",
            "strFanart1": null,
            "strFanart2": null,
...
public:
    ...
    std::string searchForVenue(const std::string venueName) override {
        return searchVenuesData;
    };

    std::string getAllSports() override { return ""; };
    std::string getAllLeagues() override { return ""; };
...
TEST(SearchServiceTest, SearchTeamsByNameReturnsData) {
    FakeLogger logger;
...
TEST(SearchServiceTest, SearchEventByNameReturnsData) {
    FakeLogger logger;
    FakeApiClient apiClient;
    SearchService service(logger, apiClient);
    std::vector<Event> result = service.searchEventByName("Arsenal_vs_Chelsea");
    EXPECT_FALSE(result.empty());
}

TEST(SearchServiceTest, SearchEventByNameAndYearReturnsData) {
    FakeLogger logger;
    FakeApiClient apiClient;
    SearchService service(logger, apiClient);
    std::vector<Event> result = service.searchEventsByNameAndYear("Arsenal_vs_Chelsea", "2016", "2017");
    EXPECT_FALSE(result.empty());
}

TEST(SearchServiceTest, SearchEventByEventFileNameReturnsData) {
    FakeLogger logger;
    FakeApiClient apiClient;
    SearchService service(logger, apiClient);
    std::vector<Event> result = service.searchEventByEventFileName("English_Premier_League_2015-04-26_Arsenal_vs_Chelsea");
    EXPECT_FALSE(result.empty());
}

TEST(SearchServiceTest, SearchForVenuesReturnsData) {
    FakeLogger logger;
    FakeApiClient apiClient;
    SearchService service(logger, apiClient);
    std::vector<Venue> result = service.searchForVenue("Wembley");
    EXPECT_FALSE(result.empty());
}
